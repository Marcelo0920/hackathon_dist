...
function ComplaintsList() {
  ...
  return (
    <div className="bg-white p-8 rounded-lg border border-gray-200 shadow-lg mt-16">
      ...
      <ul className="menu bg-white lg:menu-horizontal rounded-box">
        <li>
          <button
            className={` ${viewList ? "bg-white" : ""}`}
            onClick={() => setViewList(true)}
          >
            <p className="hover:text-green-500 focus:text-green-500">
              Vista en lista
            </p>
          </button>
        </li>
        <li>
          ...
          >
            <p className="hover:text-green-500">Vista en mapa</p>
          </button>
        </li>
        {/* <li>
          <button
      ...
      <ul className="menu bg-white lg:menu-horizontal rounded-box ">
        <li>
          ...
          >
            <p className="hover:text-green-500"> Limpiar filtro</p>
          </button>
        </li>
        <li>
          <select
          ...
          >
            <option disabled value="">
              <p className="hover:text-green-500">Tipo de denuncia</p>
            </option>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 px-3 py-4 border-b border-gray-200 text-gray-900">
          {complaints?.map((complaint) => {
            ...
            return (
              <div
                key={complaint._id}
                className="card bg-slate-300 shadow-xl shadow-green-100"
              >
                <figure>
                  <img src={complaint.photos[0]} alt={complaint.title} />
                </figure>

                {/* CAMBIAR EL COLOR DEL BOX DEPENDIENDO DEL ESTADO */}

                <div className="card-body">
                  <h2 className="card-title">{complaint.title} </h2>
                  <p>{complaint.description}</p>
                  <div className="badge badge-neutral bg-orange-500">
                    <p className="text-white">{complaint.state}</p>
                  </div>
                  <div
                    className="badge badge-neutral"
                    style={{
                      backgroundColor:
                        complaint.prioridad === "baja"
                          ? "green"
                  ...
                  >
                    <p className="text-white" style={{ padding: "5px" }}>
                      {complaint.prioridad}
                    </p>
                  </div>
                  <div className="bg-white h-2 rounded-lg overflow-hidden mt-2 mb-4">
                    <div
                  ...
                  <div className="card-actions justify-end">
                    ...
                    <button
                      className="btn btn-sm bg-green-700"
                      onClick={() => {
                        navigate(
